...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        cond_in_dim = hidden_size * 3 if self.use_structure else hidden_size * 2
        self.cond_proj = nn.Sequential(
            nn.Linear(cond_in_dim, hidden_size),
            nn.SiLU(),
    ...
    def forward(self, x, labels, t, label_mask, s, structure_mask):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        y: (N,) tensor of class labels
        label_mask: (N,) tensor indicating whether to mask out label information
        s: (N, S) embedding of structure information
        structure_mask: (N,) tensor indicating whether to mask out structure information
        ...
        if self.ordinal_input:
            ...
            #increments = self.v.weight**2
            #c = torch.cumsum(increments, dim=0)    # FOR TRAINING, SAMPLING
            #y_input = c[labels.long()]
            y_input = labels.float() # FOR INTERPOLATION SAMPLING
        else:
        ...
        if structure_mask is not None:
            s = s * (1 - structure_mask.to(torch.float32).view(-1, 1))

        if self.use_structure:
            #c = torch.cat([t, y, s], dim=1)
            c = t + y + s
        else:
